...
namespace SampleMessageGenerator.sensor
{
    public class Sensor3 : ISensor
    {
        ...
        public Sensor3(string name, IModel channel, int rangeFrom, int rangeTo, int n, int sleepTime)
        {
            Name = name;
            Channel = channel;
            RangeFrom = rangeFrom;
        ...
        public void GenerateMany()
        {
            for (int i = 0; i < N; i++)
            {
                ...
                Channel.BasicPublish(exchange: "", routingKey: "sensor3", body: Encoding.UTF8.GetBytes(json));
