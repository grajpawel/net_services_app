...
namespace SampleMessageGenerator.sensor
{
    public class Sensor1 : ISensor
    {
        ...
        public Sensor1(string name, IModel channel, int rangeFrom, int rangeTo, int n, int sleepTime)
        {
            Name = name;
            Channel = channel;
            RangeFrom = rangeFrom;
        ...
        public async void GenerateMany()
        {
            for (int i = 0; i < N; i++)
            {
                ...
                Channel.BasicPublish(exchange: "", routingKey: "sensor1", body: Encoding.UTF8.GetBytes(json));
