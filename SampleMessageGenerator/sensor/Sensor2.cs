...
namespace SampleMessageGenerator.sensor
{
    public class Sensor2 : ISensor
    {
        ...
        public Sensor2(string name, IModel channel, int rangeFrom, int rangeTo, int n, int sleepTime)
        {
            Name = name;
            Channel = channel;
            RangeFrom = rangeFrom;
        ...
        public async void GenerateMany()
        {
            for (int i = 0; i < N; i++)
            {
                ...
                Channel.BasicPublish(exchange: "", routingKey: "sensor2", body: Encoding.UTF8.GetBytes(json));
